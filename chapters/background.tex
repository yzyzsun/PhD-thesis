...
Union types are the dual concept of intersection types. While an intersection $A
\tand B$ represent a type that is both $A$ and $B$, a union $A \tor B$
represents a type that is either $A$ or $B$. A direct application of union types
is to model nullable types~\citep{nieto2020scala}, which can be represented as
$A \tor \mathbf{Null}$.

Again, if we consider their logical counterparts, a noticeable property is that
implication is right-\emph{anti}distributive over disjunction:
\begin{equation*}
...
$(\mathbf{Int}\to\mathbf{Bool})\tand(\mathbf{Double}\to\mathbf{Bool})$.
