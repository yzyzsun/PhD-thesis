...
represents a type that is either $A$ or $B$. Although tagged unions are more
common in functional languages, as seen in algebraic data types, we focus on
\emph{un}tagged unions in this thesis.
...
axiom, for example, by \citet{barbanera1995intersection}:
\begin{equation}
  (A \to C) \tand (B \to C)  \quad <: \quad  A \tor B \to C \label{eq:dist-4}
\end{equation}
A final note is that intersections and unions can distribute over each other:
\begin{align}
